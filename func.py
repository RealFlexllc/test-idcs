    ...
    if (len(oauth_apps) < 2):
        ...
        try:
            ...
            oauth_apps['idcs'] = {'introspection_endpoint': context['idcs_introspection_endpoint'], 
                                  ...
                                  'client_secret': context['idcs_app_client_secret_ocid']}
            oauth_apps['oic'] = {'token_endpoint': context['back_end_token_endpoint'], 
                                  'client_id': context['back_end_app_client_id'], 
                                  'client_secret': context['back_end_client_secret_ocid']}
            logging.getLogger().info('Initial config set completed')
...
def introspectToken(access_token, introspection_endpoint, client_id, client_secret):
    ...
    try:
        logging.getLogger().info('Entered introspect token' + access_token)
        token = requests.post(introspection_endpoint, 
                              data=payload, 
                              headers=headers, 
                              auth=HTTPBasicAuth(client_id, 
                              client_secret))
        logging.getLogger().info(token.text)                      
        logging.getLogger().info('Exit Introspect Token' )                      
...
def getBackEndAuthToken(token_endpoint, client_id, client_secret):
    ...
    payload = {'grant_type': 'client_credentials','scope':'https://D89D8E604A8E4862B5FE678A9D4C5083.integration.ocp.oraclecloud.com:443urn:opc:resource:consumer::all'}
    headers = {'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'}
    try:
...
def getAuthContext(token, client_apps):
    ...
    try:
        logging.getLogger().info('Entered getAuthContext' + token)
        ...
        logging.getLogger().info('getAuthContext Completed')
    ...
    if (token_info['active'] == True):
        ...
        client=token_info['client_name']
        if(client == "oic_promote_auth" ):
            auth_context['account']='migration_account'
        else:
            auth_context['account']='app_account'
        ...
        auth_context['context'] = {'account': ('promote_account')}
...
def handler(ctx, data: io.BytesIO=None):
    logging.getLogger().info('Entered Handle')
    initContext(dict(ctx.Config()))
    logging.getLogger().info('After InitContext')
    auth_context = {}
    try:
        
        gateway_auth = json.loads(data.getvalue())
        #logging.getLogger().info('Data received:' + gateway_auth)
        auth_context = getAuthContext(gateway_auth['token'], oauth_apps)
        logging.getLogger().info('auth_context set')
